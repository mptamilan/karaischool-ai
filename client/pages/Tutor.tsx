...
export default function TutorPage() {
  ...
  const canSend = useMemo(() => !loading && input.trim().length > 0 && usage.remaining > 0 && !!user, [loading, input, usage.remaining, user]);
  ...
  const send = async (text: string) => {
    ...
    const m: Message = { role: "user", content: text, timestamp: new Date().toISOString() };
    ...
    <div className="mx-auto max-w-7xl px-4 md:px-6 py-6 md:py-10 grid grid-cols-1 md:grid-cols-[auto,1fr] gap-6">
      ...
      <main className="space-y-4">
        <div className="glass rounded-2xl p-4 md:p-6">
          <div className="flex items-center gap-2 text-sm font-semibold mb-4"><Sparkles className="h-4 w-4"/>AI Tutor</div>
          {!user && (
            <div className="mb-4 flex items-center gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-amber-800">
              <AlertTriangle className="h-4 w-4"/>
              <div className="text-sm">Please sign in with Google to use the tutor.</div>
            </div>
          )}
          {error && (
          ...
          {messages.length === 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              {examples.map((ex) => (
                <button key={ex} onClick={() => send(ex)} className="rounded-xl border bg-white/70 p-3 text-left shadow-sm hover:shadow transition">
          ...
          <div ref={scrollRef} className="h-[55vh] md:h-[60vh] overflow-y-auto space-y-3 pr-1">
            {messages.map((m, i) => (
              <ChatMessage key={i} role={m.role} content={m.content} timestamp={m.timestamp} />
            ))}
            {loading && (
              <div className="flex justify-start">
                <div className="glass rounded-2xl px-4 py-3"><LoadingDots/></div>
              </div>
            )}
          </div>
          <div className="mt-4">
            <div className="flex items-end gap-3">
              <textarea
                ...
                onKeyDown={onKeyDown}
                placeholder={user ? "Ask anything..." : "Sign in to start chatting"}
              ...
              />
              <button disabled={!canSend} onClick={() => send(input.trim())} className="btn-secondary disabled:opacity-50 disabled:cursor-not-allowed">
                <SendHorizonal className="h-4 w-4 mr-2"/>Send
              </button>
            </div>
            <div className="mt-2 text-xs text-slate-500">{usage.remaining} requests remaining today</div>
          </div>
        </div>
      </main>
    </div>
